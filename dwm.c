...
static void spawn_notify(const Arg *arg);
...
{
	if (arg->i == 0)
		selmon->gappx = 17; //default value
	else if (selmon->gappx + arg->i < 0)
		selmon->gappx = 0; //minimum 0
	else
		selmon->gappx += arg->i;
	arrange(selmon);
}
...
void
spawn_notify(const Arg *arg){
	//executing bash crashes dwm
	/*char script[32];
    strcpy(script, "echo ");
    strcat(script, ((char**)arg->v)[0]);
    strcat(script, " | netcat -N localhost 10000");
    char *const args[] = {"bash", "-c", script, NULL};
